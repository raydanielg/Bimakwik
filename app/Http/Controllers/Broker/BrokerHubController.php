...
class BrokerHubController extends Controller
{
    ...
    {
        $totalPolicies = 0; $totalClaims = 0; $totalCommission = 0; $monthlyData = []; $reports = collect();
        try {
            ...
            $monthlyData = BrokerCommission::selectRaw("strftime('%Y-%m', created_at) as month, SUM(commission_amount) as total")
                ...
                ->orderBy('month')
                ->get();
            $reports = Report::where('created_by', Auth::id())->latest()->paginate(15);
        } catch (\Exception $e) {}
        return view('broker.reports.index', compact('totalPolicies', 'totalClaims', 'totalCommission', 'monthlyData', 'reports'));
    }
    ...
    public function exportReports()
    {
        ...
        return response()->streamDownload(function () use ($reports) {
            ...
            fclose($out);
        }, 'broker-reports.csv', ['Content-Type' => 'text/csv']);
    }
