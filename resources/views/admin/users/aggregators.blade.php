                            ...
                            <td class="text-center">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li><a class="dropdown-item" href="{{ route('admin.users.show', $aggregator) }}"><i class="bi bi-eye me-2"></i>View Details</a></li>
                                        <li><a class="dropdown-item" href="{{ route('admin.users.edit', $aggregator) }}"><i class="bi bi-pencil me-2"></i>Edit</a></li>
                                        <li>
                                            <form method="POST" action="{{ route('admin.users.language.update', $aggregator) }}">
                                                @csrf
                                                <input type="hidden" name="preferred_language" value="{{ ($aggregator->preferred_language ?? 'en') === 'en' ? 'sw' : 'en' }}">
                                                <button type="submit" class="dropdown-item">
                                                    <i class="bi bi-translate me-2"></i>Switch to {{ ($aggregator->preferred_language ?? 'en') === 'en' ? 'Kiswahili' : 'English' }}
                                                </button>
                                            </form>
                                        </li>
                                        <li><hr class="dropdown-divider"></li>
                                        <li>
                                            <form method="POST" action="{{ route('admin.users.destroy', $aggregator) }}" onsubmit="return confirm('Are you sure you want to delete this user?')">
                                                @csrf
                                                @method('DELETE')
